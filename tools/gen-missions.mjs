// turns the MISSIONS.md dump into src/data/brm5-missions.json, plus the id to name
// index src/data/mission-titles.ts that the map side links through.
// MISSIONS.md is gitignored, the files it produces are not. re-run after a re-dump:
...
const TITLES = resolve(root, 'src/data/mission-titles.ts');
const LINKS = resolve(root, 'src/data/location-missions.ts');
...
// the map side links to missions but must not pull the 180 kB json into the main
// bundle, so it gets the names on their own
const titles = Object.fromEntries(missions.map(m => [m.id, m.name]));
writeFileSync(TITLES, `// generated by tools/gen-missions.mjs, do not edit by hand
...
export const MISSION_TITLES: Record<string, string> = ${JSON.stringify(titles, null, 2)};
`);

// location-missions.ts is hand written and names missions by id, so a re-dump that
// renames or drops one would leave a dead link. say so now rather than shipping it
if (existsSync(LINKS)) {
  const linked = new Set();
  for (const [, list] of readFileSync(LINKS, 'utf8').matchAll(/\[([^\]]*)\]/g)) {
    for (const [, id] of list.matchAll(/'([^']+)'/g)) linked.add(id);
  }
  const dead = [...linked].filter(id => !titles[id]);
  if (dead.length) throw new Error(`location-missions.ts links missions that are gone: ${dead.join(', ')}`);
}

const stubs = missions.filter(m => m.stub).length;
console.log(
  `${missions.length} missions (${stubs} unreleased) of ${all.length} total ` +
  `-> ${OUT} (${(JSON.stringify(missions).length / 1024).toFixed(0)} kB), names -> ${TITLES}`
);
